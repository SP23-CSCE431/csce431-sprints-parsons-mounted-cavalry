...
RSpec.describe 'Creating an attendance', type: :feature do
    scenario 'valid inputs' do
        ...
        css = 'tr#' + user.id.to_s + " td.empty a"
        links = page.all(css)
        links[1].click
        expect(page).to have_content('John Doe')
        expect(page).to have_content('Thursday, March 16, 2023')
...
RSpec.describe 'Viewing an attendance', type: :feature do
    scenario 'valid inputs' do
        ...
        visit 'schedules/admins'
        css = 'tr#' + user.id.to_s + " td.attendance a"
        td = find(css).click

        expect(page).to have_content('John Doe')
        expect(page).to have_content('None')
        expect(page).to have_content('2023-03-15')
...
RSpec.describe 'Deleting an attendance', type: :feature do
    ...
    scenario 'schedule that has attendance is deleted' do
        user = User.create(is_admin: true, is_staff: true, first_name: 'Jane', last_name: 'Doe', classification: 'Senior', skill_level: 'Advanced', phone_number: '1234567890', email: 'j.doe@tamu.edu')
        schedule = Schedule.create(user_id: user.id, recurrence: 'MWF')
        attendance = Attendance.create(schedule_id: schedule.id, date: '2023-12-25', check_in_time: nil, purpose: 'Training')
